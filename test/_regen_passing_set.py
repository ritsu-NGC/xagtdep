"""Helper: recompute the strict-QCEC passing set from a sweep data dir.
...
the passing set only when current, existing, and proposed are all strictly
EquivalenceCriterion.equivalent. Prints a JSON report to stdout.

Not part of the build; used to regenerate test/ci_passing_set.json.
"""
import json
import sys
...
def strict_result(data_path, idx, method, meta):
    ...
    # Find the output qubit: trust the translator-declared wire first (same
    # contract as verify_all), then fall back to a brute search. Without the
    # declared-first preference, a function that degenerates to a single PI
    # (e.g. f == x0) matches on the PI wire itself — and
    # build_reference_oracle cannot encode an oracle whose target is one of
    # its own inputs, yielding a bogus not_equivalent for ALL methods.
    declared_q = gate_data.get("output_qubit",
                               meta.get(f"output_qubit_{method}"))
    best_q = -1
    if declared_q is not None and 0 <= declared_q < n_qubits:
        if V.simulate_truth_table(circuit, num_pis, declared_q) == ref_tt:
            best_q = declared_q
    if best_q < 0:
