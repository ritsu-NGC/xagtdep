...
`random_boolean_bit_mapping` from `pebbling_solver.py`.
"""
...
def build_reed_muller_network_priority_cuts(num_vars, truth_tables, var_names=None):
    ...
    NOTE ON COST: `find_best_cut` does a linear scan over the entire
    `subset_cache` for every monomial that isn't an exact cache hit.
    For small-to-moderate `num_vars` (roughly <= 8-10) and a modest
    number of output truth tables, this is fast in practice since the
    cache size is bounded by the number of distinct subsets actually
    constructed (not 2**num_vars). For much larger problems, consider
    indexing the cache by size to avoid the full scan.
    """
    net = PebblingNetwork()
