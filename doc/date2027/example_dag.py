...
disable). Use --topo-order dependency_chain to see how grouping changes
when Phase 1 prefers chaining dependent nodes together instead of
following creation order -- see pebbling_solver.py module docstring,
"TOPOLOGICAL ORDER CHOICE", for the full rationale. With this example
DAG and --max-pebbles 3, --topo-order dependency_chain groups n1 with
n3 (its actual dependent) instead of n2 (an unrelated sibling).
"""
...
def main():
    ...
    parser.add_argument("--topo-order", choices=["creation", "dependency_chain"],
                         ...
                         help="Which topological order build_gate_groups uses "
                              "for its Phase 1 greedy pebble-budget fill "
                              "(default: 'creation', i.e. today's original "
                              "behavior). Pass 'dependency_chain' to see "
                              "producer/consumer pairs (e.g. n1/n3 in this "
                              "example DAG) grouped together instead of "
                              "unrelated siblings created back-to-back (e.g. "
                              "n1/n2). See pebbling_solver.py module "
                              "docstring, 'TOPOLOGICAL ORDER CHOICE'.")
    ...
    parser.add_argument("--no-group-boxes", action="store_true",
                         help="With --tikz, don't overlay gate-group "
                              ...
                              "using the same --max-pebbles/--topo-order "
                              "gate groups computed for the rest of this "
                              "script's output).")
    ...
    gates = build_gate_groups(
        net, max_pebbles=args.max_pebbles, topo_order=args.topo_order,
    )
    print(f"\n(gate groups computed with topo_order={args.topo_order!r})")
